...
if (isset($_POST['savePoints'])){
    $juryId = $db->getUserId();
    unset($_POST['userIDofJury']);
    $projectId = str_replace(".php","",$_POST['projectId']);
    unset($_POST['projectId'],$_POST['savePoints']);
    $eval=serialize($_POST);
    $sqlString = "SELECT COUNT(*) FROM points WHERE jury_id = {$juryId} AND project_id = {$projectId} AND step = '$page'";
    var_dump($sqlString);
    $checkIfExists = $db->qSql($sqlString)->fetchColumn();
    if ($checkIfExists == 1){
        $dbSave = $db->qSql("UPDATE points SET evaluation = '{$eval}' WHERE jury_id = {$juryId} AND project_id = {$projectId} AND step = '$page'");
    }else{
        $dbSave = $db->insert("points",[
    ...
    $db->qSql("UPDATE points SET calc = {$avg} WHERE jury_id = {$juryId} AND project_id = {$projectId} AND step = '$page'");
}

$projectId = str_replace(".php","",basename($_GET['id']));
$juryId = $db->getUserId();
$checkIfExists = $db->qSql("SELECT COUNT(*) FROM points WHERE jury_id = {$juryId} AND project_id = {$projectId} AND step = '$page'")->fetchColumn();
if ($checkIfExists == 1){
    $oldPoints = unserialize($db->qSql("SELECT evaluation FROM points WHERE jury_id = {$juryId} AND project_id = {$projectId} AND step = '$page'")->fetchColumn());
}
?>
                                ...
                                <div class="col-6" style="height: fit-content;">
                                    <div class="embed-responsive embed-responsive-16by9">
                                        <video controls class="embed-responsive-item">
                                            ...
                                            ?>
                                            <source src="/files/<?=$prj['project_video_one']?>" type="<?=$mime1?>">
                                        </video>
                                    </div>
                                </div>
                                <div class="col-6" style="height: fit-content;">
                                    <div class="embed-responsive embed-responsive-16by9">
                                        <video controls class="embed-responsive-item">
                                            ...
                                            ?>
                                            <source src="/files/<?=$prj['project_video_two']?>" type="<?=$mime2?>">
                                        </video>
                                    </div>
                                </div>
                    </div>
